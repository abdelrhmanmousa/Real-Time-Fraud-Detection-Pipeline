...
usage() {
  ...
  echo "  --gcs-bucket             GCS bucket for staging and temp."
...
# aguments validation
if [[ -z "${GCP_PROJECT_ID-}" || -z "${GCS_BUCKET-}" || -z "${INPUT_TOPIC_ID-}" || -z "${MODEL_ENDPOINT_URL-}" || -z "${JOB_SOURCE_DIR-}" || -z "${SERVICE_ACCOUNT_NAME-}" ]]; then
...
STAGING_LOCATION="gs://${GCS_BUCKET}/${JOB_NAME_PREFIX}/staging/"
TEMP_LOCATION="gs://${GCS_BUCKET}/${JOB_NAME_PREFIX}/temp/"
OUTPUT_PATH="gs://${GCS_BUCKET}/raw_processed_transactions/"
