...
# def build(known_args, pipeline_args):
#     """Defines and returns a runnable Beam pipeline."""

#     options = PipelineOptions(pipeline_args)

#     options.view_as(GoogleCloudOptions).project = known_args.project_id
#     options.view_as(GoogleCloudOptions).region = (
#         known_args.region if hasattr(known_args, "region") else "us-central1"
#     )
#     options.view_as(GoogleCloudOptions).staging_location = (
#         known_args.staging_location
#         if hasattr(known_args, "staging_location")
#         else f"gs://{known_args.project_id}-dataflow-staging/staging"
#     )
#     options.view_as(GoogleCloudOptions).temp_location = (
#         known_args.temp_location
#         if hasattr(known_args, "temp_location")
#         else f"gs://{known_args.project_id}-dataflow-temp/temp"
#     )
#     options.view_as(GoogleCloudOptions).service_account_email = (
#         known_args.service_account_email
#         if hasattr(known_args, "service_account_email")
#         else None
#     )  # Use default if not specified

#     options.view_as(StandardOptions).runner = (
#         known_args.runner if hasattr(known_args, "runner") else "DirectRunner"
#     )
...
#     print(f"The current runner is >>> {known_args.runner} <<<")

#     ml_batch_size = int(known_args.ml_batch_size)
#     window_size_secs = int(known_args.window_duration_seconds)
#     allowed_lateness = int(known_args.allowed_lateness_seconds)
...
#                 endpoint_url=known_args.model_endpoint_url,
#                 project_id=known_args.project_id,
#                 use_emulator=known_args.use_emulator,
#                 emulator_host=known_args.emulator_host,
#             )
#         )
#     )
...
#     (
#         ml_results  # or raw, depending on whether you want to window on the enriched records
...
#                 base_path=os.path.join(known_args.output_path, "final"),
